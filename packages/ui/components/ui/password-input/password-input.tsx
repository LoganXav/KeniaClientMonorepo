...
const PasswordInput = forwardRef<HTMLInputElement, InputProps & { label?: string }>(({ className, label, ...props }, ref) => {
  ...
  return (
    <div className="relative">
      <Input id={label} type={showPassword ? "text" : "password"} className={cn("hide-password-toggle pr-10 peer h-9 pt-4 bg-transparent", className, !label && "pt-0 py-1")} ref={ref} pattern="[.*]" {...props} />
