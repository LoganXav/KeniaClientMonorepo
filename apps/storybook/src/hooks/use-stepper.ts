...
function reducer(state: StepperState, { type, payload }: ActionTypes): StepperState {
  switch (type) {
    case "NEXT": {
      if (payload?.step === state.step) {
        return state;
      }
      const step = payload.step && !isNaN(payload?.step) && Number(payload?.step) >= state.step ? Number(payload.step) : state.step + 1;
    ...
    case "PREVIOUS": {
      ...
      const step = payload.step && !isNaN(payload?.step) && Number(payload?.step) <= state.step && Number(payload?.step) >= 0 ? Number(payload.step) : state.step ? getNextStep(state.step - 1) : 0;
    ...
    case "RESET": {
      const step = Number(payload.step) ? Number(payload.step) : payload.initialStep || 0;
